...
def load_config(file_path):
    ...
    assert "reader_yml" in global_config['Global'],"absence reader_yml in global"
...
def merge_config(config):
    ...
    for key, value in config.items():
        if "." not in key:
            ...
            ), "the sub_keys can only be one of global_config: {}, but get: {}, please check your running command".format(global_config.keys(), sub_keys[0])
...
def build(config, main_prog, startup_prog, mode):
    ...
    """
    # fluid.program_guard(main_prog, startup_prog) 该接口应配合使用python的 with 语句来将
    # with block 里的算子和变量添加进指定的全局主程序（main program）和启动程序（startup program）。
    with fluid.program_guard(main_prog, startup_prog):
        # fluid.unique_name.guard() 使用with语句更改全局命名空间。
...
def train_eval_rec_run(config, exe, train_info_dict, eval_info_dict, is_pruning=False):
    '''
    ...
    if not os.path.exists(save_model_dir):
        os.makedirs(save_model_dir)
    train_stats = TrainingStats(log_smooth_window, ['loss', 'precision',"recall"])
    best_eval_f1 = -1
        ...
        try:
            while True:
                ...
                if config['Global']['loss_type'] != 'srn':
                    ...
                    precision,recall, acc_num, pred_num,total_num = cal_predicts_accuracy(
                ...
                t2 = time.time()
                train_batch_elapse = t2 - t1
                stats = {'loss': loss, 'precision': precision,"recall":recall}
                train_stats.update(stats)
                ...
                if train_batch_id > 0 and\
                    ...
                    if eval_f1 > best_eval_f1:
                        ...
                        best_batch_id = train_batch_id
                        best_epoch = epoch
                        best_eval_precision = metrics['avg_precision']
                        best_eval_recall = metrics['avg_recall']
                        save_path = save_model_dir + "/best_f1"
                        if is_pruning:
                            import paddleslim as slim
                            slim.prune.save_model(exe, train_info_dict['train_program'],
                                      save_path)
                            ...
                            save_model(train_info_dict['train_program'], save_path)
                    strs = 'Test iter: {}, precision:{:.6f}, recall:{:.6f}, f1:{:.6f}, best_precision:{:.6f}, best_recall:{:.6f}, best_f1:{:.6f}, best_epoch:{}, best_batch_id:{}'.format(
                        train_batch_id, metrics["avg_precision"],metrics["avg_recall"],eval_f1,  best_eval_precision,  best_eval_recall, best_eval_f1, best_epoch,
                        best_batch_id)
                    logger.info(strs)
                train_batch_id += 1
...
def preprocess():
    ...
    #  load_config Returns: global config
    ...
    #  alg in ['Rosetta', 'CRNN', 'STARNet', 'RARE', 'SRN']是rec network

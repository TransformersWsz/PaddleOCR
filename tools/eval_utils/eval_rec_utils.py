...
def eval_rec_run(exe, config, eval_info_dict, mode):
    ...
    for data in eval_info_dict['reader']():
        ...
        if config['Global']['loss_type'] != "srn":
            ...
            precison,recall, acc_num, pred_num,total_num = cal_predicts_accuracy(
        ...
        total_sample_num += total_num
        total_pred_num += pred_num
        #logger.info("eval batch id: {}, acc: {}".format(total_batch_num, acc))
        total_batch_num += 1
    avg_precision = total_acc_num * 1.0 / total_pred_num
    avg_recall = total_acc_num *1.0 / total_sample_num
    avg_f1 = 0 if (avg_precision+avg_recall) == 0 else 2.0 * avg_precision * avg_recall / (avg_precision + avg_recall)
    metrics = {'avg_precision': avg_precision, "avg_recall": avg_recall, \
               "avg_f1": avg_f1}
    return metrics

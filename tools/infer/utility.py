...
def parse_args():
    ...
    parser.add_argument("--gpu_mem", type=int, default=11000)
    ...
    parser.add_argument("--rec_batch_num", type=int, default=4)
    ...
    parser.add_argument("--use_zero_copy_run", type=str2bool, default=True)
...
def create_predictor(args, mode):
    ...
    """
    ZeroCopyTensor是AnalysisPredictor的一种输入/输出数据结构，与PaddleTensor等同。ZeroCopyTensor相比于PaddleTensor，
    可以避免预测时候准备输入以及获取输出时多余的数据拷贝，提高预测性能。
    注意: 需要注意的是，使用ZeroCopyTensor，务必在创建config时设置config.switch_use_feed_fetch_ops(False)用于显式地在模型运行的时候删去feed和fetchops，
    不会影响模型的效果，但是能提升性能。
    """

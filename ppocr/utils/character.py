...
def cal_predicts_accuracy(char_ops,
    ...
    for ino in range(len(labels_lod) - 1):
        ...
        total_num += len(labels_text)
        pred_num += len(preds_text)

        
        for i in range(min(len(labels_text),len(preds_text))):
            if labels_text[i] == preds_text[i]:
                acc_num += 1
    precision = 0 if pred_num==0 else acc_num * 1.0 / pred_num
    recall = acc_num * 1.0 / total_num
    return precision,recall, acc_num, pred_num,total_num
